...
(defun mostrar-menu-inicial ()
  ...
  (progn
   ...
   (format t " ~%|     3 - Limpar Tabela de Memoização |")
   (format t " ~%|     0 - Sair                        |")
   (format t " ~%|_____________________________________|")
   (format t " ~%                                       ")
   (format t " ~%-> Opção: ")))
  ...
  (progn
   ...
   (let ((option (read)))
     (cond
      ...
      ((eq option '3) (limpar-memoizacao) (menu-inicial))
      ((eq option '0) (format t "Até à próxima!"))
      (T (progn (format t "Opção inválida!") (menu-inicial)))))))

(defun menu-escolher-jogador ()
  "Chama a função mostrar-menu-jogador, lê o input do utilizador e redireciona para o menu respetivo"
  (progn
...
(defun jogo-humano-vs-pc (jogador-humano tempo-limite)
  "Função que gere o jogo entre o humano e o computador, gere as jogadas e alterna entre os jogadores"
  (let ((jogador 1)
        (adversario 2)
        (no-atual (no-inicial))
...
(defun jogada-computador (no jogador adversario profundidade tempo-limite)
  "Representa uma jogada do computador, chama o algortimo que possibilita a jogada"
  (progn
   (reiniciar-valores)
   (definir-limite-tempo tempo-limite)
   (definir-jogador-proprio jogador)
   (let ((valor (alfabeta
                 no
     ...
     (progn
      ...
      (if valor
          (progn
           ...
           (mostrar-estatisticas valor (obter-nos-analisados)
                                 (obter-cortes-alfa) (obter-cortes-beta)
                                 (obter-limite-tempo-alcancado) (obter-memoizacao-ativada)))
          (mostrar-jogador-passou jogador))))
   (obter-melhor-jogada)))

(defun escolher-peca ()
  "Mostra as opções para escolher uma peça e lê o input do utilizador"
  (progn
...
(defun escolher-linha ()
  "Pede ao utilizador para inserir a linha onde a jogada vai ser feita e lê o input do utilizador"
...
(defun escolher-coluna ()
  "Pede ao utilizador para inserir a coluna onde a jogada vai ser feita e lê o input do utilizador"
...
(defun jogada-humano (no jogador)
  "Representa uma jogada por parte de um jogador humano, trata da interação com o utlizador para permitir realizar uma jogadao"
  (let ((peca nil)
        (x nil)
        (y nil))
    (progn
             ...
             (progn
              ...
              (let ((casas-ocupadas (peca-casas-ocupadas x y (funcall peca))))
                (if (valida-casas (estado-tabuleiro (no-estado no)) casas-ocupadas jogador)
                    (let ((novo-no (criar-no (criar-novo-estado (no-estado no) casas-ocupadas peca jogador) (list peca (list x y)))))
                      (mostrar-turno-jogador jogador)
                      (mostrar-no novo-no)
                      novo-no
                    )
...
(defun jogo-pc-vs-pc (tempo-limite)
  "Função que gere o jogo entre o computador e o computador, gere as jogadas e alterna entre os jogadores"
  (let ((jogador 1)
        (adversario 2)
        (no-atual (no-inicial))
        (profundidade (profundidade-max-para-tempo tempo-limite)))
...
(defun mostrar-jogador-passou (jogador)
  "imprime no caso de o jogador passar a jogada"
...
(defun registar-jogador-passou (jogador)
  "regista no log.dat no caso de o jogador passar a jogada"
...
(defun mostrar-estatisticas (melhor-valor nos-analisados cortes-alfa cortes-beta limite-tempo-alcancado memoizacao-ativada)
  "imprime as estatisticas relativas a uma jogada feita pelo computador"
  ...
  (format t "Memoização ativada ~a vezes~%" memoizacao-ativada)
  (registar-estatisticas melhor-valor nos-analisados cortes-alfa cortes-beta limite-tempo-alcancado memoizacao-ativada))

(defun mostrar-turno-jogador (jogador)
  "imprime a imformação de qual o jogador que pertence o turno"
  (format t "Turno do Jogador ~a ~% ------------------- ~%" jogador)
  (registar-turno-jogador jogador))

(defun registar-turno-jogador (jogador)
  "regista a imformação de qual o jogador que pertence o turno"
  (with-open-file (file (diretorio-resultados) :direction :output :if-exists :append :if-does-not-exist :create)
    (format file "Turno do Jogador ~a ~% ------------------- ~%" jogador)))

(defun registar-estatisticas (melhor-valor nos-analisados cortes-alfa cortes-beta limite-tempo-alcancado memoizacao-ativada)
  "regista as estatisticas relativas a uma jogada feita pelo computador"
  (with-open-file (file (diretorio-resultados) :direction :output :if-exists :append :if-does-not-exist :create)
    ...
    (format file "Memoização ativada ~a vezes~%~%" memoizacao-ativada)))
...
(defun registar-pontuacoes (estado)
  "regista as Pontuações no ficheiro log.dat"
  (progn
   (with-open-file (file (diretorio-resultados) :direction :output :if-exists :append :if-does-not-exist :create)
...
(defun registar-cabecalho (tipo tempo)
  "regista o cabeçalho no ficheiro log.dat"
  (progn
   (with-open-file (file (diretorio-resultados) :direction :output :if-exists :append :if-does-not-exist :create)
     (format file "~%/////////////////////////////////////////////////////////////////////////~%")
...
(defun profundidade-max-para-tempo (milisegundos)
  "Ao receber o tempo retorna a profundidade a ser usada para esse tempo limite definido"
  (cond
   ((< milisegundos 8000) 3)
   ((< milisegundos 14000) 4)
